...
app.post(`${PREFIX}/admin/login`, async (c) => {
  ...
  try {
    ...
    // Security: Check both static environment credentials and dynamically created admin accounts
    ...
    if (!acct) {
      ...
      if (dynamicAcct) {
        // Verifyhashed password securely using SHA-256 scheme to protect dynamic accounts
        const pwHash = await sha256Hex(password + ":" + username);
        if (dynamicAcct.pwHash === pwHash) {
          acct = {
            username: dynamicAcct.username,
            password: "",
            role: dynamicAcct.role,
            totpSecret: dynamicAcct.totpSecret, // Load TOTP secret if dynamic account has one
          };
        }
      }
    }
...
app.post(`${PREFIX}/admin/login/2fa`, async (c) => {
  ...
  try {
    ...
    // Security: Check both static and dynamic accounts for TOTP matching to prevent bypass
