...
async function logWebhookEvent(opts: {
  ...
  try {
    ...
    const sensitive = new Set(["authorization", "x-user-token", "x-admin-token", "x-agent-2fa-token", "x-kkiapay-secret", "x-fedapay-signature", "x-callback-key", "x-cinetpay-signature", "x-token"]);
    try {
      ...
      if (raw && typeof raw.forEach === "function") {
        raw.forEach((v: string, k: string) => {
          ...
          headers[k] = sensitive.has(key) ? "[REDACTED]" : v.slice(0, 500);
        });
      } else if (opts.c?.req?.header) {
        const tracked = ["content-type", "user-agent", "x-forwarded-for", "x-kkiapay-secret", "x-fedapay-signature", "x-callback-key", "x-cinetpay-signature", "x-token", "authorization"];
        for (const h of tracked) {
          const v = opts.c.req.header(h);
          if (v) headers[h] = sensitive.has(h.toLowerCase()) ? "[REDACTED]" : String(v).slice(0, 500);
        }
      }
    } catch { /* ignore */ }
