...
app.post(`${PREFIX}/admin/login`, async (c) => {
  ...
  try {
    ...
    // Unified account lookup (static environment accounts first, then dynamic admin roles from KV)
    let acct: { username: string; role: "superadmin" | "operator" | "support"; totpSecret?: string } | null = null;
    const staticAcct = ADMIN_ACCOUNTS.find((a) => a.username === username && a.password === password);
    if (staticAcct) {
      acct = staticAcct;
    } else {
      const roles = ((await kv.get(k.adminRoles())) ?? []) as any[];
      const dynamicRole = roles.find((r) => r.username === username);
      if (dynamicRole) {
        const hash = await sha256Hex(password + ":" + username);
        if (dynamicRole.pwHash === hash) {
          acct = {
            username: dynamicRole.username,
            role: dynamicRole.role,
            totpSecret: dynamicRole.totpSecret,
          };
        }
      }
    }
...
app.post(`${PREFIX}/admin/login/2fa`, async (c) => {
  ...
  try {
    ...
    // Dynamic accounts with 2FA support
    const staticAcct = ADMIN_ACCOUNTS.find((a) => a.username === payload.username);
    let acct: { username: string; role: "superadmin" | "operator" | "support"; totpSecret?: string } | null = null;
    if (staticAcct) {
      acct = staticAcct;
    } else {
      const roles = ((await kv.get(k.adminRoles())) ?? []) as any[];
      const dynamicRole = roles.find((r) => r.username === payload.username);
      if (dynamicRole) {
        acct = {
          username: dynamicRole.username,
          role: dynamicRole.role,
