...
app.post(`${PREFIX}/admin/dev/seed-demo`, async (c) => {
  ...
  try {
    ...
    // SECURITY: Generate cryptographically secure random bytes for demo client password
    const passwordBuf = new Uint8Array(8);
    crypto.getRandomValues(passwordBuf);
    const password = `Demo!${b64urlEncode(passwordBuf).slice(0, 8)}`;
